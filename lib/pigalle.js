...
class Pigalle extends PigalleBaseClass {
  ...
  registerPigallePlugin (plugin, alias) {
    ...
    return this.addScopeToPlugin(plugin)
  }
  ...
  registerThirdPartyPlugin (plugin, alias) {
    ...
    return this.addScopeToThirdParty(plugin)
  }

  addRootScopeFn(plugin) {
    return () => {
      plugin.pigalle = () => this
      return plugin
    }
  ...
  addScopeToPlugin (plugin) {
    const scope = plugin.scope()
    const name = plugin.name()

    const fn = this.addRootScopeFn(plugin)
  ...
  addScopeToThirdParty (plugin) {
    plugin.pigalle = this.addRootScopeFn(plugin)
    return this
  }
